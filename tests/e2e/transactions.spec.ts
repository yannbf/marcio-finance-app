...
test.describe("Transactions full history", () => {
  test("renders header, search input, filter pills", async ({ page }) => {
    ...
    // Filter pills are links — labels in i18n are "All / Categorized /
    // Uncategorized" rather than the older "Matched / Unmatched".
    await expect(
      page.getByRole("link", { name: /All/ }).first(),
    ).toBeVisible();
    await expect(
      page.getByRole("link", { name: /Categorized/ }),
    ).toBeVisible();
    await expect(
  ...
  test("Unmatched filter narrows the list", async ({ page }) => {
    ...
    // ING Hypotheken (mortgage) is matched in the seed → should NOT appear.
    await expect(page.getByText("ING Hypotheken")).toHaveCount(0);
